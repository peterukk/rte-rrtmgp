...
This script generates training data and is meant to be run from /examples/nndev
in the RTE+RRTMGP library
...
# 1. Choose which variable to predict
# So far following choices are supported:
...
#   "sw_ray"--> Rayleigh optical depth / cross-section (SW), used for computing
#   scattering by gas molecules

predictand = "lw_abs"
# predictand = "lw_planck"
# predictand = "sw_abs"
# predictand = "sw_ray"


# 2. Choose data source or data generation method 
# data_source = ["Ukkonen2020", "synthetic"]
# "Ukkonen2020" = Generate inputs/outputs from existing netCDF datasets used in Ukkonen
# et al 2020. This is an extensive hybrid data set which covers a wide range of atmospheric
# conditions and gas concentratinos. It was generated by first obtaining
# realistic atmospheric profiles from various sources (reanalysis, GCM, etc) and then
# extending this data by artificially sampling the concentrations of all gases
# except H2O and O3 in multidimensional space by using e.g. Halton sequences. 
# The aim was to sample the hypercube evenly, where the hypercube has 14 dimensions
# in the longwave due to many minor gases. 
# The data covers many "experiments" for each dataset which correspond not only
# to to the hypercube samples but also augmented temperatures/ water vapor, e.g.
# "+4K, constant RH" (these experiments were inspired by those in the RFMIP dataset).
# 
# "synthetic" = create input data on-the-fly by choosing a set of gases
# to vary in a user-defined range, as well as the range of temperatures
# 
data_source  = "Ukkonen2020"
# data_source = "synthetic" # Not yet implemented!




if predictand == "lw_abs" or "lw_planck":
    shortwave = False
elif predictand == "sw_abs" or "sw_ray":
    shortwave = True
else:
    print("Did not understand choice of predictand; check the code")


# In the longwave we're interested in absorption (optical depth) and emission,
# for which we use Planck fraction
# In the shortwave we're interested in absorption and Rayleigh absorption

# predictand      = 1  # = 0 for planck fraction (longwave) or ssa(shortwave), 
...
# frac_cams       = 0.85 # What fraction of CAMS data to keep by random selection
frac_val        = 0.025  # What fraction of datasets to use for validation
seed            = 7
less_gases      = False # Only use CKDMIP gases as inputs
remove_CKDMIP_exps  = False  # Remove the experiments which have non-CKDMIP gases set to zero and use CFC11eq
garand_val = True
...
x_tr, x_val, x_test, y_tr, y_val, y_test, ngas, ngpt, inds = load_data_all(fname_rfmip,
              fname_cams, fname_nwp, fname_GCM, fname_Garand, fname_CKDMIP,
              include_cams, include_nwpsaf, include_GCM, include_Garand, include_CKDMIP,
              shortwave, predictand, dcol,frac_val, frac_cams,  seed, 
              remove_CKDMIP_exps, garand_val)
gc.collect()
