...
def build_y2(rsd_s,rsu_s,rsd0,albedo):
    ns = rsd_s.shape[0]
    rsd_pred = np.concatenate((np.ones((ns,1)), rsd_s),axis=1)
    rsu_pred = np.concatenate((rsu_s, albedo*rsd_pred[:,-1:]),axis=1)
    for i in range(nlay+1):
        rsd_pred[:,i] = rsd_pred[:,i] *rsd0
        rsu_pred[:,i] = rsu_pred[:,i] *rsd0
    return rsd_pred, rsu_pred
...
if scale_outputs: 
    ...
    fac = 2.625
    y_tr= np.copy(y_tr_raw); y_val= np.copy(y_val_raw) 
    # y_test = np.copy(y_test_raw)
    y_tr[:,:,1] = fac*y_tr[:,:,1] 
    y_val[:,:,1] = fac*y_val[:,:,1] 
    # y_test[:,:,1] = fac*y_test[:,:,1] 
else:
    y_tr = y_tr_raw; y_val = y_val_raw; 
    #y_test = y_test_raw
...
if use_auxinputs:
    ...
    if mu0_and_albedo_as_auxinput:
        mlp_dense_inp2 = Dense(nneur, activation=activ0,name='dense_inputs_mu0')(inp_aux_mu)
...
fpath = 'saved_model/bigru_gru_16'
...
start = time.time()
y_pred_val = sess.run(["dense_output"], {"inputs_aux_albedo": xaux, "inputs_main": x_val_m})[0]
end = time.time()
print(end - start)
gc.collect()
...
rsd_pred_val, rsu_pred_val = build_y2(y_pred_val[:,:,0],y_pred_val[:,:,1], rsd0_val, x_val_aux1)
