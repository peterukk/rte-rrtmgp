...
if mu0_as_layer_input:
    ...
    x_tr_aux = x_tr[:,0,-1]; x_val_aux = x_val[:,0,-1]; x_test_aux = x_test[:,0,-1]    
    nx_aux = 1
    ...
    x_tr_aux1 = x_tr_aux[:,0:1]; x_tr_aux2 = x_tr_aux[:,1:]; 
    x_val_aux1 = x_val_aux[:,0:1]; x_val_aux2 = x_val_aux[:,1:]; 
    x_test_aux1 = x_test_aux[:,0:1]; x_test_aux2 = x_test_aux[:,1:]; 
...
if not mu0_as_layer_input: inp_aux1 = Input(shape=(nx_aux),name='inputs_aux1') # mu0
inp_aux2 = Input(shape=(nx_aux),name='inputs_aux2') # sfc_albedo
...
if mu0_as_layer_input:
    mlp_dense_inp1 = Dense(nneur, activation=activ0,name='dense_inputs_aux1')(inp_aux2)
else:
    mlp_dense_inp1 = Dense(nneur, activation=activ0,name='dense_inputs_aux1')(inp_aux1)
    
mlp_dense_inp2 = Dense(nneur, activation=activ0,name='dense_inputs_aux2')(inp_aux2)
...
if mu0_as_layer_input:
    model = Model(inputs=[inputs, inp_aux2, target, dpres, incflux], outputs=outputs)
else:
    model = Model(inputs=[inputs, inp_aux1, inp_aux2, target, dpres, incflux], outputs=outputs)
# model.add_metric(math_ops.reduce_sum(x), name='metric_1')

model.add_metric(rmse_hr(target,outputs,inp_aux2,dpres,incflux),'rmse_hr')
# model.add_metric(rmse_flux(target,outputs,inp_aux,incflux),'rmse_flux')

model.add_loss(CustomLoss(target,outputs,inp_aux2,dpres, incflux))
