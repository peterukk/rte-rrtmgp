...
# -*- coding: utf-8 -*-
"""
Retrieve CAMS data for radiation computations
...
#dlfolder = '/media/peter/samlinux/gdrive/phd/data/CAMS/CAMS_ml_%s0201.grb'%(year)
dlfolder = '/media/peter/samlinux/data/CAMS/'
...
for month in ["02", "05", "08", "11"]:
    ...
    c.retrieve(
        ...
        dlfolder+'CAMS_eac4_sfc_%s%s01.grb'%(year,month))
    
    dict_eac4 = get_dict_eac4_ml(year,month)
    c.retrieve(
        'cams-global-reanalysis-eac4', dict_eac4,
        dlfolder+'CAMS_eac4_ml_%s%s01.grb'%(year,month))
    
    # EGG4
    dict_egg4 = get_dict_egg4_sfc(year,month)
    c.retrieve(
        'cams-global-ghg-reanalysis-egg4', dict_egg4,
        dlfolder+'CAMS_egg4_sfc_%s%s01.grb'%(year,month))
    
    dict_egg4 = get_dict_egg4_ml(year,month)
    c.retrieve(
        'cams-global-ghg-reanalysis-egg4', dict_egg4,
        dlfolder+'CAMS_egg4_ml_%s%s01.grb'%(year,month))


# PREPROCESS INTO ONE NetCDF FILE USING CDO COMMANDS
# these are in a bash script
# The final output is netcdf/CAMS_${year}.nc
os.chdir(dlfolder)
os.system("./preprocess_CAMS {}".format(year)) 
...
# processing
dlfolder_n2o = '/media/peter/samlinux/data/CAMS/N2O/'
os.chdir(dlfolder_n2o)
fname = dlfolder_n2o+'CAMS_n2o_%s.tar.gz'%(year)
...
dat = Dataset(dlfolder_n2o+fname_tmp,'a')
...
fname_final = "CAMS_{}_n2o.nc".format(year)
...
"-d time,476 -d time,720 -d time,724 {} {}".format(fname_tmp2,fname_final) 
os.system(codestr) 
os.system("rm *tmp*")

# FINALLY, concatenate N2O and main data files
os.chdir(dlfolder)
os.system("ncks -A /N2O/{} netcdf/CAMS_{}.nc".format(fname_final,year))
os.system("ncatted -h -a history,global,d,, netcdf/CAMS_{}.nc".format(year))
os.system("ncatted -h -a history_of_appended_files,global,d,, netcdf/CAMS_{}.nc".format(year))