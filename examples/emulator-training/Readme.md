...
3. After compiling the ml_allsky program, it can be used in different ways depending on arguments. Data can be generated by running scripts/ml_gendata.py (modify this Python script to change file names and other arguments). 

It can also be run manually (compute shortwave fluxes using refererence code):

` ./ml_allsky_sw 480 data_input/CAMS_2011_RFMIPstyle.nc ../../../rrtmgp/data/rrtmgp-data-sw-g224-2018-12-04.nc`

Additionally save input output data for training machine learning emulators:

` ./ml_allsky_sw 480 data_input/CAMS_2011_RFMIPstyle.nc ../../../rrtmgp/data/rrtmgp-data-sw-g224-2018-12-04.nc data_training/ml_data_ng224_CAMS_2011_RFMIPstyle.nc`

Use existing ML emulator, e.g. neural network to accelerate RRTMGP kernel (the only emulator currently supported):

` ./ml_allsky_sw 480 data_input/CAMS_2011_RFMIPstyle.nc ../../../rrtmgp/data/rrtmgp-data-sw-g224-2018-12-04.nc rrtmgp ../../neural/data/BEST_tau-sw-abs ../../neural/data/BEST_tau-sw-ray`

If it's not run in data generation mode, fluxes will be saved to a reference file (this file, whether to include clouds, and the cloud coefficient file can be specified by modifying the Fortran program). 
