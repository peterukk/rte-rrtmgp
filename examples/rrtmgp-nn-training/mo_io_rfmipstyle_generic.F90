...
!
! -------------------------------------------------------------------------------------------------
module mo_io_rfmipstyle_generic
...
contains
  ...
  subroutine read_and_block_sw_bc(fileName, blocksize, &
    ...
    if(nf90_inq_varid(ncid, "total_solar_irradiance", varid) /= NF90_NOERR) then 
      ...
      allocate(total_solar_irradiance(blocksize,nblocks))
  ...
  subroutine read_and_block_gases_ty(fileName, blocksize, gas_names, names_in_file, gas_conc_array)
    ...
    !
    ! New version: gases are NOT necessarily a function of experiment only, check using if statement
    ! This also means that water vapor and ozone don't require special treatment
    !
  ...
  !   Output arrays are allocated within this routine
  !
  subroutine read_and_block_clouds_cams(fileName, blocksize, &
                               clwc, ciwc, cloud_fraction)
    character(len=*),           intent(in   ) :: fileName
    integer,                    intent(in   ) :: blocksize
  ...
  subroutine unblock_and_write_2D_dp(fileName, varName, values)
    character(len=*),           intent(in   ) :: fileName, varName
    real(dp), dimension(:,:),  & ! [blocksize, nblocks]
                                intent(in   ) :: values
    ! ---------------------------
    integer :: ncid
    integer :: b, blocksize, nlev, nblocks
    ...
    ncid = nf90_close(ncid)
    deallocate(temp1d)
  end subroutine unblock_and_write_2D_dp

  subroutine unblock_and_write_2D_sp(fileName, varName, values)
    character(len=*),           intent(in   ) :: fileName, varName
  ...
  !   in an existing netCDF file.
  !
  subroutine unblock_and_write_3D_dp(fileName, varName, values)
    character(len=*),           intent(in   ) :: fileName, varName
    real(dp), dimension(:,:,:),  & ! [nlay/+1, blocksize, nblocks]
                                intent(in   ) :: values
    ! ---------------------------
    integer :: ncid
    integer :: b, blocksize, nlev, nblocks
    ...
    ncid = nf90_close(ncid)
    deallocate(temp2d)
  end subroutine unblock_and_write_3D_dp

  subroutine unblock_and_write_3D_sp(fileName, varName, values)
    character(len=*),           intent(in   ) :: fileName, varName
    real(sp), dimension(:,:,:),  & ! [nlay/+1, blocksize, nblocks]
                                intent(in   ) :: values
    ! ---------------------------
    integer :: ncid
    integer :: b, blocksize, nlev, nblocks
  ...
  subroutine unblock_and_write_4D_dp(fileName, varName, values)
    ...
    real(sp), dimension(:,:,:), allocatable, target :: temp3D
    real(sp), dimension(:,:,:,:), contiguous, pointer  :: output4D
    ...
    call C_F_POINTER (C_LOC(temp3D), output4D, [nfirst, nlev, ncol_l, nexp_l])

    call stop_on_err(write_field(ncid, varName,  output4D))
  ...
  subroutine unblock_and_write_4D_sp(fileName, varName, values)
    ...
    call C_F_POINTER (C_LOC(values), output4D, [nfirst, nlev, ncol_l, nexp_l])
  ...
  !   end if
  ! end subroutine
end module mo_io_rfmipstyle_generic
