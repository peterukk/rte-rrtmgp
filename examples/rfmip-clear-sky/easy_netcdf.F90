...
module easy_netcdf
  ...
  use netcdf
  use mo_rte_kind,           only: wp, sp, dp, wl, i4

!   use parkind1,      only : wp, i4, dp
!   use radiation_io,  only : nulout, nulerr

  implicit none
  public

  integer, parameter :: nulout = 6, nulerr = 6
  ...
  contains
    ...
    procedure, public :: end_define_mode, start_define_mode
...
contains
    ...
    if(error_msg /= "") then
      ...
      write (error_unit,*) "easy_netcdf stopping"
      stop
    end if
  ...
  ! is read-only)
  subroutine open_netcdf_file(this, file_name, iverbose, is_write_mode, redefine_existing, is_hdf5_file)
    class(netcdf_file)            :: this
    character(len=*), intent(in)  :: file_name
    integer, intent(in), optional :: iverbose
    logical, intent(in), optional :: is_write_mode
    logical, intent(in), optional :: redefine_existing
    ...
    if (present(redefine_existing)) this%is_write_mode = .true.
    ...
    if (.not. this%is_write_mode) then
      ...
      if (present(redefine_existing)) then
        if (redefine_existing) then
          istatus =  nf90_open(file_name, NF90_WRITE, this%ncid)


          if (istatus /= NF90_NOERR) then
            write(nulerr,'(a,a,a,a)') '*** Error opening existing file for writing ', &
                 &    trim(this%file_name), ': ', trim(nf90_strerror(istatus))          
          end if

          istatus = nf90_redef(this%ncid)
          if (istatus /= NF90_NOERR) then
            write(nulerr,'(a,a,a,a)') '*** Error starting define mode for ', &
                 &    trim(this%file_name), ': ', trim(nf90_strerror(istatus))
            call stop_on_err('Error writing NetCDF file')
          end if
        end if
      else
    ...
    if (istatus /= NF90_NOERR) then
      ...
      call stop_on_err('Error opening NetCDF file')
    end if
  ...
  subroutine get_variable_id(this, var_name, ivarid)
    ...
    if (istatus /= NF90_NOERR) then
      ...
      call stop_on_err('Error reading NetCDF file')
    end if
  ...
  subroutine get_array_dimensions(this, ivarid, ndims, ndimlens, ntotal)
    ...
    integer(kind=i4), intent(out), optional :: ntotal
  ...
  subroutine start_define_mode(this)
    class(netcdf_file)             :: this
    integer                        :: istatus
    if (this%is_define_mode) then
      istatus = nf90_redef(this%ncid)
      if (istatus /= NF90_NOERR) then
        write(nulerr,'(a,a,a,a)') '*** Error starting define mode when writing ', &
             &    trim(this%file_name), ': ', trim(nf90_strerror(istatus))
        call stop_on_err('Error writing NetCDF file')
      end if
      this%is_define_mode = .true.
    end if
  end subroutine start_define_mode
  ...
  subroutine get_real_matrix(this, var_name, matrix, do_transp)
    ...
    real(wp), allocatable      :: tmp_matrix(:,:)
  ...
  subroutine get_real_array3(this, var_name, var, ipermute)
    ...
    real(wp), allocatable   :: var_permute(:,:,:)
  ...
  subroutine put_real_vector(this, var_name, var)
    ...
    call this%get_variable_id(var_name, ivarid)
    call this%get_array_dimensions(ivarid, ndims, ndimlens, ntotal)
    if (ntotal /= size(var,kind=i4)) then
  ...
  subroutine put_real_matrix(this, var_name, var, do_transp)
    ...
    character(len=*), intent(in)   :: var_name
    real(wp), intent(in)         :: var(:,:)
    real(wp), allocatable        :: var_transpose(:,:)
    ...
    if (ntotal /= size(var,kind=i4) .and. ntotal /= 0) then
  ...
  subroutine copy_variable(this, infile, var_name)
    ...
    real(kind=dp), allocatable :: data_real(:)
