...
if (nblocks==1) then
  ...
  ! Compare fluxes to benchmark result, alongside reference RTE+RRTMGP computations?
  ...
  function mean_3d(x3) result(mean3)
    ...
    mean3 = sum(sum(sum(x3, dim=1),dim=1),dim=1) / (size(x3, dim=1)*size(x3,dim=2)*size(x3,dim=3))
